...
void Game::init_game() {
    ...
    for (Player &player : players) {
        if (index % 4 == 0) {
            ...
            player.head = { num_cells_x - 1,
                            r * num_cells_y / (num_on_right + 1) };
            r++;
        } else if (index % 4 == 2) {
            ...
            player.head = { d * num_cells_x / (num_on_down + 1),
                            num_cells_y - 1 };
            d++;
        }
        player.ai_player =
                ...
                for (int j = 0; j < num_players;
                    ...
                    std::chrono::high_resolution_clock::time_point
                        end_of_waiting =
                            ...
                            time_for_one_round;
                    for (int k = 0;
                         k < (int)std::thread::hardware_concurrency(); ++k) {
                        if (j + k >= num_players) {
                            break;
                        }
                        if (players[j + k].ai_plays &&
                            !players[j + k].game_over) {
                            players[j + k].ai_player->init_move(grid,
                                                                player_states);
                            futures.emplace_back(Threadpool::submit_contract(
                                &AI::move, players[j + k].ai_player.get(),
                                end_of_waiting));
                        }
                    }
                    ...
                    for (auto &future : futures) {
                        while (!(players[index].ai_plays &&
                                 !players[index].game_over)) {
                            index++;
                        }
                    ...
                    for (auto &future : unresponsive_futures) {
                        auto status =
                            future->wait_for(unresponsive_ai_wait_time);
                        if (status != std::future_status::ready) {
